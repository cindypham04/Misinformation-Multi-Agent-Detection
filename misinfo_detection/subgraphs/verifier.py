...
def _call_ollama_verifier(state: VerifierState) -> tuple[VerdictLabel, str] | None:
    ...
    prompt = (
        ...
        "IMPORTANT: Judge the factual accuracy of the CLAIM, not the quality of the debating.\n"
        "'supported' means the claim IS TRUE — the majority of credible evidence corroborates it.\n"
        "'refuted' means the claim IS FALSE — the majority of credible evidence contradicts it.\n"
        "'insufficient' means no reliable sources exist, OR authoritative sources directly contradict each other on the core fact.\n"
        "Do NOT confuse 'supported' with 'the affirmative debater argued well' — focus on what the evidence says about the claim itself.\n"
        "Prefer 'supported' or 'refuted' over 'insufficient' whenever fact-checking organizations "
        "(PolitiFact, Snopes, AP News, Reuters, CDC, WHO) clearly favor one side.\n"
        'Return strict JSON with the shape {"verdict":"supported|refuted|insufficient","report":"short explanation"}.\n'
        f"Claim: {state['claim']}\n"
        f"Debate log: {json.dumps([e[:200] for e in state.get('debate_log', [])], ensure_ascii=True)}\n"
